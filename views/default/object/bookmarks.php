...
$link_options = array('href' => $href);

if ($link_behaviour == "blank_external") {
	if ((stripos($href, "http:") === 0) || (stripos($href, "https:") === 0)) {
		if (stristr($href, elgg_get_site_url()) === false) {
			$link_options["target"] = "_blank";
			$link_options["title"] = elgg_echo("bookmarks_tools:entity:href:new_window");
		}
	}
}

$link = elgg_view('output/url', $link_options);

if ($link_behaviour == "blank_icon") {
	$link .= elgg_view('output/url', array(
			"text" => elgg_view_icon("bookmarks-tools-new-window"),
			"href" => $href,
			"target" => "_blank",
			"title" => elgg_echo("bookmarks_tools:entity:href:new_window")
		));
}
